...
def main():
    ...
    # ── Save full point map (N, H, W, 3) float32 ─────────────────────────────
    # NaNs kept intentionally — direct pixel->3D lookup for counting pipeline.
    # Shape is recoverable from array itself: pointmap.shape = (N, H, W, 3)
    np.save(f"{output}_pointmap.npy", point_map_np.astype(np.float32))
    print(f"Point map saved: {output}_pointmap.npy  "
          f"({point_map_np.nbytes / 1e6:.1f} MB)")

    # ── Save confidence map (N, H, W) float32, normalised 0-1 ────────────────
    conf_map = conf_norm.reshape(N, H, W).astype(np.float32)
    np.save(f"{output}_confmap.npy", conf_map)
    print(f"Conf map saved:  {output}_confmap.npy  "
          f"({conf_map.nbytes / 1e6:.1f} MB)")
    ...
    print(f"\nDone! Outputs:")
    print(f"  {output}.ply")
    print(f"  {output}_pointmap.npy")
    print(f"  {output}_confmap.npy")
    print(f"  {output}_filenames.txt")
