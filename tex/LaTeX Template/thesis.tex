...
%\end{itemize}
Automatic fruit counting based on images is a key task in precision agriculture, where accurate yield estimates support harvest planning and resource allocation. \todo{citation}
Reconstructing a fruit tree scene in 3D using a series of RGB images and counting the fruits it contains is appealing because it requires only inexpensive and widely available camera hardware, rather than specialized depth sensors. 

Recent work such as FruitNeRF \cite{meyer2024fruitnerf} shows that this is possible by training a Neural Radiance Field \cite{mildenhall2020nerf} per scene and counting the fruits in the resulting volumetric representation. This approach is effective but requires a time-consuming, scene-specific optimization. In this work, we instead investigate whether a feed-forward multi-view stereo model, VGGT \cite{wang2025vggt}, can replace this scene-by-scene training. Instead of optimizing a representation of each scene, VGGT predicts a 3D reconstruction in a single forward pass. This promises a significant reduction in runtime, but raises the central question of wether the resulting geometry is sufficiently accurate for reliable counting.
