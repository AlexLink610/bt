...
%\end{itemize}
Feeding a growing world population is a major global challenge, and projections estimate that total food demand will rise substantially over the coming decades \cite{vandijk2021foodsecurity}. Precision agriculture responds to this by using advanced technology and data analysis to maximize crop yields while reducing waste and environmental impact \cite{karunathilake2023precision}. Within precision agriculture, counting the fruit on a tree is an important building block: it provides the yield estimates that growers rely on to plan the harvest and allocate labor, storage, and other resources \cite{miranda2023}. Because performing the count manually across many trees is slow and labor-intensive, automating it from images is a valuable capability.

Among the ways to automate fruit counting, reconstructing the tree in 3D from a set of RGB images is particularly attractive, for two reasons. First, lifting the count into a shared three-dimensional representation lets each fruit be counted exactly once, instead of being counted repeatedly across the many overlapping images in which it appears. Second, capturing the necessary data requires only an ordinary color camera, of the kind built into any consumer device, rather than the specialized and more expensive depth-sensing equipment, such as LiDAR scanners or calibrated stereo rigs, on which other 3D-capture methods depend. This makes the approach inexpensive and easy to deploy in the field.
\todo[inline]{citation for cost, other 3D capture methods}

Recent work such as FruitNeRF \cite{meyer2024fruitnerf} shows that image-based 3D fruit counting is feasible by training a Neural Radiance Field \cite{mildenhall2020nerf} per scene and counting the fruit in the resulting volumetric representation. This approach is effective, but it requires a time-consuming optimization for every new scene. In this work, we instead investigate whether a feed-forward multi-view stereo model, VGGT \cite{wang2025vggt}, can replace this per-scene training. Rather than optimizing a representation for each scene, VGGT predicts a 3D reconstruction in a single forward pass. This promises a significant reduction in runtime, but raises the central question of whether the resulting geometry is accurate enough for reliable counting.

\section{Challenges}

Counting fruit reliably from images of a tree faces a number of difficulties, some inherent to the trees themselves and some arising from the way they must be captured.

The trees are difficult subjects in their own right. Fruit are frequently occluded, both by foliage and by other fruit, so that any single image captures only part of the crop. Their appearance changes with the viewing angle and with lighting conditions such as direct sunlight or shadow, which makes the same fruit look different when seen from different viewpoints. In addition, the branches, leaves and fruit of a tree canopy form a geometrically complex, cluttered structure that is hard to reconstruct consistently.

The way the tree is captured adds further difficulties. To limit occlusion and avoid missing fruit, a tree should be observed from all sides. The number of images that can be used per tree is bounded, however, by the memory available to the reconstruction model. (see Section \ref{sec:vggt_advantages}). To still achieve full coverage, the cameras must therefore be placed sparsely around the tree, spanning a full 360-degree range. As a consequence, neighboring images are taken from widely spaced viewpoints and often show markedly different regions of the tree, with little visual content in common. This makes it hard to establish \emph{correspondences} between images, that is, to determine which parts of one image show the same physical points as another. The problem is compounded by the scene not being perfectly static during capture: wind moves branches, leaves, and fruit between one image and the next, so that even points that are visible in both images no longer align, further degrading the cross-view correspondences on which the later counting steps rely. These unreliable correspondences are the root cause of the counting errors this thesis addresses: depending on the situation, the same apply may be split into several counts or several apples merged into one. How these errors arise in our pipeline, and how we counteract them, is discussed in Chapter \ref{chap:method}.
...
Each input image $I_i$ is first divided into a grid of non-overlapping image patches. Each patch is then encoded into a \emph{token}, a fixed-length vector that represents the local content of that patch and serves as the basic unit on which the network operates \cite{dosovitskiy2021vit}. In VGGT, this patch encoding is performed by a pretrained DINOv2 backbone \cite{oquab2024dinov2}. An image is thus converted into a set of tokens, each of which carries the local appearance and structural information of its region. For each image, the patch tokens are augmented with additional learnable tokens: a \emph{camera token}, which aggregates the information needed to predict the camera parameters of this image, as well as a set of \emph{register tokens}, which serve as auxiliary memory during the attention calculation and are discarded before the final prediction.

\paragraph{Alternating Attention.}
The core of the network is a transformer \cite{vaswani2017attention} that processes all tokens of all images jointly. It consists of $24$ attention blocks that alternate between two types of self-attention. In \emph{frame-wise} self-attention, tokens attend only to other tokens of the same image, which preserves and refines the local structure of each image. In \emph{global} self-attention, all tokens attend to one another across all images, thereby exchanging information between views and establishing the correspondences that link the individual images into a single, coherent scene. By alternating between these two modes, the network baalnces local detail with global multi-view consistency. This alternating attention design is the central architectural mechanism of VGGT; the authors show that it is superior to both purely global attention and cross-attention alternatives \cite{wang2025vggt}.
...
\section{RANSAC}
\label{sec:ransac}
Fitting a geometric model to a set of measured points is a recurring subtask in our pipeline. Whenever such measurements originate from imperfect detectors or reconstructions, they contain not only small measurement noise but also \emph{gross errors}: points that are simply wrong rather than slightly inaccurate. A conventional estimator such as least squares offers no protection against these. As an averaging technique, it relies on the assumption that any deviation from the model is a small measurement error and that such deviations balance each other out over a sufficiently large number of points \cite{fischler1981ransac}. Coarse errors clearly violate this assumption, and even a single severly misplaced point can distort the entire fit. To robustly estimate a model under such conditions, we use RANSAC (Random Sample Consensus). 

RANSAC was introduced by Fischler and Bolles \cite{fischler1981ransac} and reverses the logic of conventional smoothing: Instead of fitting the model to all points and then attempting to remove those that do not match, it fits the model to a small sample and then measures how many of the remaining points match it. Each  proceeds in three steps. First, it draws a \emph{minimal sample}, the smallest number of points that suffices to instantiate the model, and computes the model from this sample alone. Second, it tests the instantiated model against all other points and collects every point that lies within a fixed error tolerance of it into the \emph{consensus set} for this sample; points outside the tolerance are considered outliers. Third, the size of the consensus set is recorded as the value for this sample. These steps are repeated over a specified number of iterations, and the model with the largest consensus set is retained. Optionally, this final model is refitted to all points in its consensus set to refine the estimate \cite{fischler1981ransac}. 

The robustness of the method stems from this construction. Since the model is selected by consensus among many points rather than by minimizing a residual across all points, gross outliers that would derail an averaging estimator are excluded from the consensus set and therefore do not influence the retained model. Figure \todo{figure ransac} illustrates this on the simple case of fitting a circle to a set of 2D points: three points determine a candidate circle, and the points within the tolerance band form its consensus set, while the outliers that would bias a least-squares fit are left out.
\missingfigure{RANSAC circle fitting illustration}

\section{Clustering}
\label{sec:clustering}
The final step in our pipeline groups a set of points in 3D space so that points that belong together are combined into a single point and counted only once. Given a set of points, the task is to divide them into clusters so that points that are spatially close to one anotther belong to the same cluster, while points that are far apart belong to different clusters. The number of resulting clusters is the measure of interest. We consider two families of methods for this grouping.

\subsection{Density-Based Clustering}
An obvious first choice is DBSCAN, a density-based algorithm introduced by Ester et al. \cite{ester1996dbscan}. Its central idea is that clusters are regions of high point density separated from one another by regions of low density. Density is evaluated based on a neighborhood radius $\varepsilon$ and a minimum number \textit{MinPts}: A point is a \emph{core point} if at least \textit{MinPts} points lie within a distance of $\varepsilon$ from it \cite{ester1996dbscan}. In practice, only $\varepsilon$ needs to be chosen, since the authors report that \textit{MinPts} can be set to a default value, so that the algorithm has only a single free parameter.
Clusters are then formed from core poitns based on a concept of \emph{density-reachability}: A point is reachable from a core point if it can be connected to it by a chain of points in which each successive point lies within $\varepsilon$ of the previous one and is itself a core point \cite{ester1996dbscan}. A cluster is the maximum set of points that are densely connected to one another, and points that are not reachable from any core point are referred to as noise.
This chaining allows DBSCAN to discover clusters of any shape, but it can be a disadvantage if clusters are expected to be spatially compact. Since membership propagates transitively along chains of $\varepsilon$-close points, two dense groups ar ekept separate from each other only if the distance between them is greater than $\varepsilon$ \cite{ester1996dbscan}. If such groups are close to one another, the points of one may lie within $\varepsilon$ of the other, and DBSCAN then connects them via the intervening points to a single cluster. Different groups are thereby linked into a single one. Reducing $\varepsilon$ to break such chains is not a solution: it also separates loosely connected points within a single group, thereby splitting a cluster into several. In general, there is no single radius that separates neighboring groups while simultaneouusly uniting the points of each individual group.

\subsection{Agglomerative Clustering with complete Linkage}
We therefore use agglomerative hierarchical clustering, which controls the spatial etent of a cluster directly rather than via a density chain \cite{hastie2009elements}. The methods begins with every point in its own cluster and repeatedly merges the two closest clusters, where the distanc ebetween two clusters is defined by a \emph{linkage} criterion \cite{hastie2009elements}. Under \emph{complete linkage}, this distance is the largest distance between any pair of points, one from each cluster; two clusters are thus merged only if \emph{all} of their points are mutually close. Merging stops once the smallest inter-cluster distance exceeds a fixed threshold.

Because complete linkage bounds the maximum distance within a cluster, it caps the diameter a cluster may attain and therefore cannot chain a sequence of points into an arbitrarily long group, as density-based clustering does. Choosing the threshold to match the expected size of a group lets the points of one group merge whil keeping separate gorups apart. The number of clusters that remain is the count reported by the method.
\missingfigure{chaining dbscan vs clusters with complete linkage }
